...
const double INFERENCE_INTERVAL = 1.00;
...
int main (int argc, char** argv) {
    ...
    // Set up the camera
    capture.open("rtsp://192.168.1.121:554/ch0_1.h264");
	...
	if (frame_count++ >= SKIP_AFTER)
            ...
            for(int i = 0; i < faces.size(); i++) {
                ...
                if (enableGenderNetwork) {
                    ...
                    if (currentInferenceResult.genderConfidence >= MALE_GENDER_THRESHOLD) {
                        ...
                        system("echo hombre desconocido | festival --tts --language spanish");
                        ...
                        system("echo mujer desconocida | festival --tts --language spanish");
                    } else {
                ...
                if (enableAgeNetwork) {
                    ...
                    string cmd("echo ");
                    cmd += currentInferenceResult.ageCategory.substr( currentInferenceResult.ageCategory.length() - 2 );
                    cmd += " anyos de edad | festival --tts --language spanish";
                    system(cmd.c_str());
        ...
        for(int i = 0; i < faces.size(); i++) {
            ...
            //system((resultText[i]).c_str());
            // char hello = (resultText[i]).c_str();
            // system("echo" + hello + "| festival --tts --language spanish");
        }
